...
B1 会话记忆还包括 Track 的横向缩放、纵向 key 高度、主 ToolMode、独立 ValueTraceShape、Lanes 显隐/最后有效高度及 List 显隐/宽度；每个 Segment 独立记忆滚动位置、活动 Lane、Lane 顺序/显隐/纵轴和 List 行位置。SubVoice 独立拥有上述全部适用状态。关闭 Tab 释放 VM/资源而保留纯描述；删除 owner 清除其自身描述，Undo 不自动重开或恢复已删除描述。普通切页/重开保留位置，显式导航按 §18.2.2 优先。纯状态不进入音乐 Modified/Undo/编译或 canonical，不触发全源重建。B2 schema 3 将这些已批准描述及 Track/Usage/Root Mute/Solo 写入 `settings/project-presentation.json`；B3 schema 4 另保存有序 Workspace Tabs、Active Workspace、批准的子页/筛选和 viewport 纯值。打开只物化 Arrangement 与 Active Workspace，其余 Tab 以有界轻量描述懒激活；用户动作优先，失效活动项回退 Arrangement。打开不自动修复损坏 presentation，显式 Save 才写回；旧 schema 1/2/3 读入空 navigation。
